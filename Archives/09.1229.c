...
#define WAR_NOTSAVED    "[!]  Current document not saved yet. Press <Enter> to continue or <Esc> to cancel."
#define HELPLINE        " F1:Help   F2:Open   F3:Save   F4:New   F10:Quit        RTF Writer for DOS " INFO_VER
#define HELPCON         "Home/End: beginning/end of the line.  PgUp/PgDn: scroll up/down one screen. Shift & move: select characters.      Ctrl+B for Bold, Ctrl+I for Italic format.\n\nRTF Writer for DOS 0.1." INFO_BUILD " " INFO_MS
/* formatting macros */
#define bold(A)         ((A) = (A) | 1)
...
int dispdoc()
{
    ...
    /*#if DEBUG
        ...
        for (col = 0;col < MAX_H; col++)
            cprintf("<%c|%d>",doc[line + headline][col].c,doc[line + headline][col].f);
    #endif*/
...
int writefile(char *path)
{
    ...
    else
    {
        ...
        fputc('}',fp);
        fclose(fp);
        if(remove(path) == -1 || rename(tmppath, path) == -1)
        {
            notice(ERR_SAVE2);
            return 0;
        }
