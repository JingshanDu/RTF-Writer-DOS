...
int scoll(int lines) /* up+ down- */
{
    ...
    {
        if(pheadline - lines + MAX_V <= MAX_L)
            pheadline -= lines;
    }
    dispdoc();
}
...
int ins(char c)
{
    ...
    for ((pt = ctail+1, ph = chead); pt->c != EOF; (pt++, ph++))
    {
        if (pt->c != '\n')
            chrcpy(ph,pt);
        else
        {
            chrcpy(ph,pt);
            pt = doc[(pt-doc) / MAX_H + 1]-1;
            ph = doc[(ph-doc) / MAX_H + 1]-1;
        }
    }
    ph->c = EOF;
    chead->c = c;
...
{
    chead = doc + x-1 + (pheadline+y-1)*MAX_H;
    ctail = doc + x-1 + (pheadline+y-1)*MAX_H;
}
/* commands */
int help()
{
