...
int edit()
{
    ...
    do
    {
        ...
        /* Line, Col */
        gotoxy(60,1);
        printf("Line %3d | Col %3d",ny+pheadline,nx);
        /* Format Indicator */
        if (chead == ctail)
        {
            if (isbold(chead->f))
...
int ins(char c)
{
    ...
    chead->c = c;
    chead->f = (chead - 1)->f;   /* so that the format will be the same as the former character */
    chead++;
    ctail = chead;
    gotoxy((chead-(chr*)doc)%MAX_H+1, (chead-(chr*)doc)/MAX_H+1-pheadline);
    dispdoc();
}
